...
data_list = ["total_cases", "new_cases", "total_deaths", "new_deaths", "total_cases_per_million",
...
class numbered_dic:
    def __init__(self, alist):
        self.alist = {idx+1 : alist[idx] for idx in range(len(alist))}
    def printdic(self):
        for key, val in self.alist.items():
            if key%3 == 0:
                isend = '\n'
            else:
                isend = ''
            entry_to_print = str(key)+') '+parse_title(val)+'  '
            print('{:<30}'.format(entry_to_print), end=isend)
